...
def infoTable( conn ):
	...
	for i in range(num_products):
		c.execute("SELECT unix_time, product_id, item_price, "
			  + "num_vended "
			  + "FROM product_readings "
			  + "WHERE product_id=%s "
			  + "ORDER BY unix_time LIMIT 1", [i])
		...
		c.execute("SELECT unix_time, product_id, item_price, "
			  + "num_vended "
			  + "FROM product_readings "
			  + "WHERE product_id=%s AND unix_time >= %s "
			  + "ORDER BY unix_time "
			  + "LIMIT 1", [i, time.time() - 86400] )
		...
		c.execute("SELECT unix_time, number "
			  + "FROM refills "
			  + "WHERE product_id=%s "
			  + "ORDER BY unix_time DESC LIMIT 1", [i]);
		...
		c.execute("SELECT num_vended FROM product_readings "
			  + "WHERE product_id=%s AND unix_time < %s "
	...
	# Temperature
	c.execute("SELECT * FROM temperatures ORDER BY unix_time DESC LIMIT 1");
	row = c.fetchone()
	if row is None:
